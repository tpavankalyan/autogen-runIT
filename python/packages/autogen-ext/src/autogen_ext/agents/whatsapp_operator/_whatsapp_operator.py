...
from ._events import WhatsappOperatorEvent
from ._prompts import (
    WHATSAPP_OPERATOR_QA_PROMPT,
...
class WhatsappOperatorConfig(BaseModel):
    name: str
    ...
    headless: bool = True
    start_page: str | None = "https://web.whatsapp.com/"
...
class WhatsappOperator(BaseChatAgent, Component[WhatsappOperatorConfig]):
    """
    WhatsappOperator is a multimodal agent that acts as an operator that can read, send and analyse messages on whatsapp.

    Installation:

    .. code-block:: bash

        pip install "autogen-ext[whatsapp-operator]"

    It launches a chromium browser with Whatsapp open in it and allows the playwright to interact with whatsapp and can perform a variety of actions. The browser is launched on the first call to the agent and is reused for subsequent calls.
    ...
    component_config_schema = WhatsappOperatorConfig
    component_provider_override = "autogen_ext.agents.whatsapp_operator.WhatsappOperator"

    DEFAULT_DESCRIPTION = """
    A helpful assistant with access to Whatsapp in a web browser.
    Ask them to read messages, search contacts, send messages and interact with content (e.g., clicking links, scrolling the viewport, filling in form fields, etc.).
    It can also summarize the messages, or answer questions based on the messages.
    It can also be asked to sleep and wait for messages or whatsapp to load, in cases where the page seems not yet fully loaded.
    """
    DEFAULT_START_PAGE = "https://web.whatsapp.com/"
    ...
    def __init__(
        ...
        """
        Initialize the WhatsappOperator.
        """
    ...
    async def _generate_reply(self, cancellation_token: CancellationToken) -> UserContent:
        ...
        if self._model_client.model_info["vision"]:
            text_prompt = WHATSAPP_OPERATOR_TOOL_PROMPT_MM.format(
            ...
            text_prompt = WHATSAPP_OPERATOR_TOOL_PROMPT_TEXT.format(
                state_description=state_description,
                visible_targets=visible_targets,
                other_targets_str=other_targets_str,
                focused_hint=focused_hint,
    ...
    async def _summarize_page(
        ...
        messages.append(SystemMessage(content=WHATSAPP_OPERATOR_QA_SYSTEM_MESSAGE))
        prompt = WHATSAPP_OPERATOR_QA_PROMPT(title, question)
    ...
    def _to_config(self) -> WhatsappOperatorConfig:
        return WhatsappOperatorConfig(
            name=self.name,
    ...
    @classmethod
    def _from_config(cls, config: WhatsappOperatorConfig) -> Self:
        return cls(
            name=config.name,
